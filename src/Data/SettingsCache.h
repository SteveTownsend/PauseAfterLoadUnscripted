...
public:
  ...
  // time out the pause - set in seconds - 0.0 means wait for user input before
  // resuming
  [[nodiscard]] double ResumeAfter() const { return _resumeAfter; }
  // delay acceptance of unpause input - set in seconds - 0.0 means allow
  // immediate unpause
  ...
  // input filtering
  [[nodiscard]] bool IgnoreButton() const {
    return _ignoreButton;
  }
  [[nodiscard]] bool IgnoreKeyPress() const {
    return _ignoreKeyPress;
  }
...
private:
  ...
  static constexpr bool DefaultIgnoreButton = false;
  static constexpr bool DefaultIgnoreKeyPress = false;
  ...
  bool _ignoreButton = DefaultIgnoreButton;
  bool _ignoreKeyPress = DefaultIgnoreKeyPress;
