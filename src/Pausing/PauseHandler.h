...
// Pause logic per wsPauseQuestAliasScript.psc, courtesy of wskeever and
// bobbyclue
#include <format>
...
class PauseHandler : public RE::BSTEventSink<RE::MenuOpenCloseEvent> {
public:
  PauseHandler(const PauseHandler &) = default;

  PauseHandler(PauseHandler &&) = default;

  PauseHandler() : _timer(_io_context), _thread() {
    _listener = std::make_unique<InputListener>(
        std::bind(&PauseHandler::Unpause, this));
    Register();
    LoadData();
  }
  ...
  // set when game loading message is detected, to distinguish fast-travel and
  // door transition from game load
  ...
  bool StartPause(const bool isSaving = false) {
    ...
    if (!TryEnterCriticalSection(reinterpret_cast<LPCRITICAL_SECTION>(
            &RE::MenuTopicManager::GetSingleton()->criticalSection))) {
      REL_WARNING("Cannot freeze time while MenuTopicManager is locked");
      return false;
    }
    // Check for known crashes
    if (RE::MenuTopicManager::GetSingleton()->lastSelectedDialogue &&
        RE::MenuTopicManager::GetSingleton()
            ->lastSelectedDialogue->parentQuest) {
      const auto quest(RE::MenuTopicManager::GetSingleton()
                           ->lastSelectedDialogue->parentQuest);
      std::string questName;
      if (quest->GetFullNameLength()) {
        questName =
            std::string(quest->GetFullName(), quest->GetFullNameLength());
      } else {
        const char *edid(quest->GetFormEditorID());
        questName = edid ? std::string(edid) : std::string();
      }
      REL_WARNING("Cannot freeze time while MenuTopicManager indicates QUST "
                  "dialogue {}/0x{:08x}",
  ...
  void ProgressPause() {
    ...
    if (controls->IsPOVSwitchControlsEnabled() &&
        ...
        // mapped this over from the script, see
        // https://github.com/SteveTownsend/PauseAfterLoadUnscripted/issues/12
      ...
      REL_WARNING("Controls-Enabled State not all true: fighting {} looking {} "
                  "journal {} menu {} movement {} sneaking {}",
                  controls->IsFightingControlsEnabled(),
                  controls->IsLookingControlsEnabled(),
                  controls->IsMainFourControlsEnabled(),
                  controls->IsMenuControlsEnabled(),
...
protected:
  ...
  ProcessEvent(const RE::MenuOpenCloseEvent *a_event,
    ...
    if (intfcStr && a_event && a_event->menuName == intfcStr->loadingMenu) {
      if (!a_event->opening) {
        // skip ProgressPause() if this is first pass after process launch, per
        // log output above
        if (_canPause) {
          // Loading Menu closed - need to pause
          REL_MESSAGE(
              "Loading Menu closed after preceding Opened event - pause OK");
          ProgressPause();
          _canPause = false;
        } else {
          REL_MESSAGE(
              "Loading Menu closed without preceding Opened event - no pause");
        }
      }
...
private:
  ...
  // indicates not first pass after launch - menu-closed must be preceded by
  // menu-opened
