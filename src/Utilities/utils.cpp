...
std::wstring GetGamePath(void) {
  ...
  if (s_runtimeDirectory.empty()) {
    ...
    uint32_t runtimePathLength = REX::W32::GetModuleFileNameW(
        REX::W32::GetModuleHandleW((const wchar_t *)(NULL)), runtimePathBuf,
        sizeof(runtimePathBuf));
    ...
    REL_MESSAGE("GetGamePath result: {}",
                StringUtils::FromUnicode(s_runtimeDirectory));
  }
...
std::wstring GetPluginPath() noexcept {
  ...
  if (s_skseDirectory.empty()) {
    ...
    if (!pluginFileName.empty()) {
      ...
      if (_wsplitpath_s(pluginFileName.c_str(), drive, MAX_PATH, dir, MAX_PATH,
                        nullptr, 0, nullptr, 0) == 0) {
        wchar_t path[MAX_PATH];
        memset(path, 0, MAX_PATH);
        if (_wmakepath_s(path, drive, dir, nullptr, nullptr) == 0) {
          s_skseDirectory = path;
          REL_MESSAGE("GetPluginPath result: {}",
                      StringUtils::FromUnicode(s_skseDirectory));
        }
      }
    }
  }
...
unsigned long long microsecondsNow() {
  ...
  if (s_use_qpc) {
    ...
    return static_cast<unsigned long long>((1000000LL * now.QuadPart) /
...
void LogProcessWorkingSet() {
  ...
  if (GetProcessMemoryInfo(GetCurrentProcess(), &pmc, sizeof(pmc))) {
    REL_MESSAGE("Working Set is {:0.3f} MB",
...
void TakeNap(const double delaySeconds) {
  ...
  if (delaySeconds < 0.0 || delaySeconds > PragmaticLongestWait) {
    REL_ERROR("Adjust requested wait of {:0.2f} seconds to {:0.2f}",
              delaySeconds, PragmaticLongestWait);
  }
  REL_MESSAGE("wait for {} milliseconds",
              static_cast<long long>(delaySeconds * 1000.0));
  ...
  auto nextRunTime =
      ...
      std::chrono::milliseconds(static_cast<long long>(delaySeconds * 1000.0));
  std::this_thread::sleep_until(nextRunTime);
}

ScopedTimer::ScopedTimer(const std::string &context)
    : m_startTime(microsecondsNow()), m_context(context) {}
...
int ScopedTimerFactory::StartTimer(const std::string &context) {
  ...
  m_timerByHandle.insert(
      std::make_pair(handle, std::make_unique<ScopedTimer>(context)));
  return handle;
#else
  return 0;
#endif
...
bool Replace(std::string &str, const std::string target,
...
std::string FromUnicode(const std::wstring &input) {
  ...
  int size_needed = REX::W32::WideCharToMultiByte(
      REX::W32::CP_UTF8, 0, &input[0], static_cast<int>(input.size()), NULL, 0,
      0, 0);
  if (size_needed == 0)
    return std::string();

  std::string output(static_cast<size_t>(size_needed), 0);
  int result(REX::W32::WideCharToMultiByte(REX::W32::CP_UTF8, 0, &input[0],
                                           static_cast<int>(input.size()),
                                           &output[0], size_needed, 0, 0));
  if (result == 0)
    return std::string();
